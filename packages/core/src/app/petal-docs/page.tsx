    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div id="overview" className="scroll-mt-6 mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div>
            <p className="text-xs text-muted-foreground mb-2 font-medium uppercase tracking-wide">New Petal host</p>
            <CodeBlock code="npx create-petal-app my-petal-host" />
          </div>
          <div>
            <p className="text-xs text-muted-foreground mb-2 font-medium uppercase tracking-wide">Scaffold a custom app</p>
            <CodeBlock code="npx @petal/cli create my-app" />
          </div>
        </div>
      ...
      <div className="flex gap-10">
        ...
        <div className="flex-1 min-w-0 space-y-16">
          ...
          <section id="host-setup" className="scroll-mt-6">
            ...
            <h3 className="font-semibold text-foreground mb-3">Step 1 · Scaffold</h3>
            <CodeBlock label="terminal" code={`npx create-petal-app my-petal-host
# or: pnpm create petal-app my-petal-host
...
├── petal.config.ts     # Primary config — apps, backend URL, theme
...
└── package.json        # @petal/sdk, @petal/ui, next`} />
            ...
            <p className="text-sm text-muted-foreground mb-3">
              This is the single source of truth for your host. <Mono>withPetal(config)</Mono> in <Mono>next.config.js</Mono> reads it at build time and injects <Mono>FRAPPE_URL</Mono>, <Mono>PETAL_APPS</Mono>, and <Mono>PETAL_THEME</Mono> as Next.js env vars automatically.
            </p>
            <CodeBlock label="petal.config.ts" code={`import type { PetalConfig } from "@petal/sdk"

const config: PetalConfig = {
  backend: "http://localhost:8000",   // Frappe backend URL
...
# Frappe site hostname for virtual-host routing
...
NEXT_PUBLIC_FRONTEND_URL=http://localhost:3000`} />
          ...
          <section id="installation" className="scroll-mt-6">
            ...
            <CodeBlock label="terminal" code={`npx @petal/cli create my-app
cd my-app
pnpm install`} />
...
pnpm start    # → http://localhost:3000`} />
            ...
            <CodeBlock label="terminal" code={`# Install the Petal CLI once
npm install -g @petal/cli

# In your deployment directory, run the interactive setup
petal setup
...
docker compose up`} />
            ...
            <CodeBlock label="generated files" code={`petal.config.ts     # apps + theme (used when building from source)
.env.local          # FRAPPE_INTERNAL_URL, NEXT_PUBLIC_FRAPPE_SITE, NEXT_PUBLIC_FRONTEND_URL
docker-compose.yml  # pulls ghcr.io/petalframework/petal:latest`} />
  ...
  petal:
    ...
    restart: unless-stopped`} />
            ...
            <p className="text-sm text-muted-foreground mb-3">
              The pre-built image reads <Mono>PETAL_APPS</Mono> from the container environment at startup. Add it to <Mono>.env.local</Mono> alongside the other secrets:
            </p>
            <CodeBlock label=".env.local (Docker)" code={`FRAPPE_INTERNAL_URL=http://frappe:8000
NEXT_PUBLIC_FRAPPE_SITE=erp.mycompany.com
...
PETAL_APPS=[
  {
    ...
    "url": "https://cdn.mycompany.com/hr-portal/2.1.0/petal.hooks.js"
            ...
            <div className="mt-3">
              <Callout variant="tip">
                Restart the container (<Mono>docker compose restart</Mono>) after updating <Mono>.env.local</Mono> — no rebuild required.
              </Callout>
            </div>
          </section>
          ...
          <section id="multi-tenancy" className="scroll-mt-6">
            ...
            <div className="space-y-2 mb-6 text-sm text-muted-foreground">
              {[
                { arrow: "→", from: "/api/config", to: "returns tenant-specific apps + theme" },
            ...
            <PropTable rows={[
              ...
              { name: "theme",      type: "Partial<PetalTheme>",              desc: "Theme overrides (primaryColor, borderRadius, fontFamily) applied to this tenant." },
  ...
  caddy:
    ...
    # Add a second Caddyfile entry for beta.example.com in production`} />
          ...
          <section id="env-vars" className="scroll-mt-6">
            ...
            <PropTable rows={[
              { name: "FRAPPE_INTERNAL_URL",       type: "string", required: true, desc: "Internal network URL of the Frappe backend used for server-to-server proxy calls (Docker/VPC). Falls back to FRAPPE_URL." },
              { name: "NEXT_PUBLIC_FRAPPE_SITE",   type: "string",                 desc: "Frappe site hostname for virtual-host routing. Defaults to the hostname parsed from the backend URL." },
              { name: "NEXT_PUBLIC_FRONTEND_URL",  type: "string",                 default: "http://localhost:3000", desc: "Public URL of this Petal instance." },
            ]} />

            <h3 className="font-semibold text-foreground mt-8 mb-3">Runtime / Docker overrides</h3>
            <p className="text-sm text-muted-foreground mb-3">
              For source builds these are injected by <Mono>withPetal()</Mono> from <Mono>petal.config.ts</Mono>. For Docker deployments, set them directly in <Mono>.env.local</Mono>.
            </p>
            <PropTable rows={[
              { name: "FRAPPE_URL",     type: "string",      required: true,  desc: "Public URL of the Frappe backend. Used by the server-side proxy for source builds. Ignored when PETAL_TENANTS is set." },
              { name: "PETAL_APPS",     type: "JSON string",                  default: "[]", desc: 'JSON array of PetalAppMeta objects. Source builds derive this from petal.config.ts. Docker deployments set it directly. Ignored when PETAL_TENANTS is set.' },
              { name: "PETAL_THEME",    type: "JSON string",                  default: "{}", desc: 'JSON object with global theme overrides. Source builds derive this from petal.config.ts. Ignored when PETAL_TENANTS is set.' },
              { name: "PETAL_TENANTS",  type: "JSON string",                  default: "—",  desc: 'Enables multi-tenant mode. JSON object keyed by hostname → TenantConfig. When set, overrides FRAPPE_URL, PETAL_APPS, and PETAL_THEME. See Multi-Tenancy section.' },
            ]} />

            <p className="text-sm text-muted-foreground mt-6 mb-3">Full <Mono>.env.local</Mono> example for Docker:</p>
            <CodeBlock label=".env.local" code={`# ── Frappe backend ──────────────────────────────────────────────
FRAPPE_URL=https://erp.mycompany.com
FRAPPE_INTERNAL_URL=http://frappe:8000        # inside Docker network

# ── Site ────────────────────────────────────────────────────────
NEXT_PUBLIC_FRAPPE_SITE=erp.mycompany.com
NEXT_PUBLIC_FRONTEND_URL=https://petal.mycompany.com

# ── Apps (Docker only — source builds use petal.config.ts) ──────
PETAL_APPS=[
  {
    "name": "hr-portal",
    "version": "2.1.0",
...
# ── Theme (Docker only — source builds use petal.config.ts) ─────
PETAL_THEME={"primaryColor":"#7c3aed","borderRadius":"0.75rem"}`} />
          </section>
    ...
    afterRequest: async (response) => {
      ...
      return response
          ...
          <section id="config-reference" className="scroll-mt-6">
            ...
            <PropTable rows={[
              { name: "backend", type: "string",          required: true,  desc: "Frappe backend URL. Injected as FRAPPE_URL by withPetal()." },
              { name: "apps",    type: "PetalAppMeta[]",  required: true,  desc: "Apps to load at runtime. Injected as PETAL_APPS by withPetal()." },
              { name: "theme",   type: "Partial<PetalTheme>", default: "{}", desc: "Global theme overrides applied before app-level overrides. Injected as PETAL_THEME." },
            ]} />
            ...
            <h3 className="font-semibold text-foreground mt-8 mb-3">PetalTheme</h3>
            <PropTable rows={[
              { name: "primaryColor", type: "string", default: "#16a34a", desc: "CSS color value. Mapped to --primary CSS variable." },
              { name: "borderRadius", type: "string", default: "0.5rem",  desc: "Mapped to --radius CSS variable." },
              { name: "fontFamily",   type: "string",                     desc: "CSS font-family string applied globally." },
            ]} />
          </section>
  ...
  sidebar_items: [
    ...
    { label: "My Feature", path: "/my-feature", icon: "my-feature" },
          ...
          <section id="cli" className="scroll-mt-6">
            ...
            <p className="text-muted-foreground mb-6">
              Two CLI tools ship with Petal: <Mono>create-petal-app</Mono> scaffolds a new host, and <Mono>@petal/cli</Mono> manages custom apps and host configuration.
            </p>

            <div className="space-y-4">
              {([
                {
                  cmd: "npx create-petal-app [project-name]",
                  badge: "Host",
                  badgeColor: "blue" as const,
                  desc: "Scaffold a new Petal host. Copies the core Next.js template, rewrites package.json with published versions, generates petal.config.ts and .env.local from interactive prompts, and rewrites tsconfig.json and tailwind.config.ts for standalone use.",
                  example: `npx create-petal-app my-petal-host
# ✓ Created my-petal-host/
# Next: cd my-petal-host && pnpm install && pnpm dev`,
                },
                {
                  cmd: "petal setup",
                  badge: "Host",
                  badgeColor: "blue" as const,
                  desc: "Interactive setup for an existing Petal deployment. Generates petal.config.ts (apps + theme), .env.local (backend secrets), and docker-compose.yml. Run this in the directory where you want to run Docker Compose.",
                  example: `petal setup
...
# Next: docker compose up`,
                ...
                {
                  cmd: "npx @petal/cli create <app-name>",
                  badge: "App",
                  ...
                  example: `npx @petal/cli create hr-portal
# ✓ Created hr-portal/
